...
export const integrationService = {
    getGoogleAuthUrl: async (params: { client_id: string; client_secret: string; redirect_uri: string }) => {
        const response = await apiClient.post<{ url: string }>('/api/v1/core/integrations/google/auth-url/', params);
        return response.data;
    },

    googleExchange: async (params: { client_id: string; client_secret: string; code: string; redirect_uri: string }) => {
