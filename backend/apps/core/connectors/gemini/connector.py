...
class GeminiConnector(BaseConnector):
    ...
    def _execute_generate_text(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        prompt = inputs.get('prompt')
        model = inputs.get('model', 'gemini-pro')
        temperature = inputs.get('temperature', 1.0)
        max_tokens = inputs.get('max_tokens', 2048)
        ...
        try:
            ...
            if hasattr(response, 'candidates') and response.candidates:
                finish_reason = response.candidates[0].finish_reason.name if hasattr(response.candidates[0], 'finish_reason') else 'STOP'
            ...
            logger.info(
                f"Gemini text generation completed",
                extra={
                    'model': model,
