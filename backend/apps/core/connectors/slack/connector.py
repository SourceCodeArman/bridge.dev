...
class SlackConnector(BaseConnector):
    ...
    def _initialize(self) -> None:
        ...
        try:
            ...
            if not response["ok"]:
                raise ValueError(
                    f"Slack authentication failed: {response.get('error', 'unknown_error')}"
                )
    ...
    def _send_message_with_retry(
        ...
        try:
            ...
            if "ratelimited" in error_str or "rate limit" in error_str:
                ...
                logger.error(
                    f"Slack message send failed: {str(e)}", extra={"channel": channel}
                )
                raise
    ...
    def _execute_list_channels(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {"channels": formatted_channels, "count": len(formatted_channels)}
