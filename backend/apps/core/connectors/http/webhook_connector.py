...
class WebhookConnector(BaseConnector):
    ...
    def _prepare_payload(self, payload: Any, step_context: Optional[Dict[str, Any]] = None) -> str:
        """
    ...
    def _execute(self, action_id: str, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if signature_method != 'none':
            ...
            if not secret:
                raise ValueError(f"secret is required for signature method {signature_method}")
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.info(
                    ...
                    extra={'url': url, 'status_code': response.status_code, 'retries': retry_count}
                )
                ...
                if attempt < max_retries:
                    ...
                    raise Exception(f"Webhook request failed after {max_retries + 1} attempts: {str(e)}")
            
            except requests.exceptions.RequestException as e:
                # Don't retry on other request exceptions (4xx errors, etc.)
                error_msg = f"Webhook request failed: {str(e)}"
                logger.error(error_msg, extra={'url': url, 'error': str(e), 'status_code': getattr(e.response, 'status_code', None)})
