...
Provides HTTP request capabilities for agent tools with URL templating,
...
class HTTPToolConnector(BaseConnector):
    ...
    def get_manifest(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to load HTTP Tool connector manifest: {str(e)}")
            # Return minimal manifest
            return {
                "id": "http-tool",
                "name": "HTTP Tool",
                "version": "1.0.0",
                "description": "Make HTTP requests for agent tools",
                "author": "Bridge.dev",
    ...
    def _initialize(self) -> None:
        """Initialize HTTP Tool connector (no special setup needed)"""
        # HTTP Tool connector doesn't need special initialization
    ...
    def _execute(self, action_id: str, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action_id in ("request", "action", ""):
            ...
            logger.warning(
                f"Unexpected action_id '{action_id}' for HTTP Tool connector, defaulting to GET"
            )
            method = "GET"
