...
from apps.accounts.permissions import IsWorkspaceMember, HasCredentialPermission, IsWorkspaceAdmin
...
class WorkflowViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def drafts(self, request, pk=None):
        ...
        if request.method == 'GET':
            ...
            draft_version = workflow.versions.filter(is_active=False).order_by('-created_at').first()
                    ...
                    if not validation_result['valid']:
                        ...
                        for field, field_errors in validation_result['field_errors'].items():
                            for error in field_errors:
                                node_validation_errors.append(f"Node {node_id}, field {field}: {error}")
            ...
            if draft_version:
                ...
                max_version = workflow.versions.aggregate(
        ...
        try:
            ...
            if not is_valid:
                return Response(
                    {
                        'status': 'error',
                        'data': {'validation_errors': errors, 'definition': workflow_definition},
            ...
            return Response({
                ...
                'data': {
                    ...
                    'validation_warnings': node_validation_errors if node_validation_errors else []
                },
    ...
    def validate_node(self, request, pk=None):
        ...
        try:
            editor = NodeEditor()
            validation_result = editor.validate_node_config(connector_id, action_id, config)
            ...
            return Response({
                ...
                'message': 'Node validation completed' if validation_result['valid'] else 'Node validation failed'
            })
        except Exception as e:
            logger.error(
    ...
    def publish_version(self, request, pk=None):
        ...
        workflow.versions.exclude(id=version.id).filter(is_active=True).update(is_active=False)
...
class ConnectorViewSet(viewsets.ReadOnlyModelViewSet):
        ...
        for connector_id in connector_ids:
            try:
                ...
                connectors.append({
                    ...
                    'is_custom': bool(manifest.get('is_custom', False)),
                })
            except Exception as e:
                logger.warning(
        ...
        if workspace:
            ...
            for custom in custom_connectors:
                ...
                connectors.append({
                    ...
                    'version': manifest.get('version'),
                    'description': manifest.get('description') or custom.description,
                    'author': manifest.get('author') or custom.created_by.email if custom.created_by else None,
                    'connector_type': manifest.get('connector_type'),
    ...
    @action(detail=True, methods=['get'], url_path='actions/(?P<action_id>[^/.]+)/form_schema')
...
class ErrorSuggestionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    @action(detail=False, methods=['get'], url_path='runs/(?P<run_id>[^/.]+)/steps/(?P<step_id>[^/.]+)')
        ...
        try:
            ...
            if not suggestion.actionable:
                return Response(
                    {
                        'status': 'error',
                        'message': 'This suggestion is not actionable'
                    },
                    status=status.HTTP_400_BAD_REQUEST
                )
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not workflow_version_id:
            return Response(
                {
                    'status': 'error',
                    'message': 'workflow_version is required'
                },
                status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if presence.user != request.user:
            return Response(
                {
                    'status': 'error',
                    'message': 'You can only remove your own presence'
                },
                status=status.HTTP_403_FORBIDDEN
            )
...
class CustomConnectorVersionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = CustomConnectorVersion.objects.filter(
            connector__workspace=workspace
        )
    ...
    def approve(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            connector.versions.filter(status='approved').exclude(id=version.id).update(status='deprecated')
