...
class RunOrchestrator:
    ...
    def create_run(
        ...
        Args:
            ...
            check_limits: Whether to check concurrency and rate limits

        Returns:
            Run instance (existing if idempotency_key matches, new otherwise)

        Raises:
            ValidationError: If workflow is not active or limits exceeded
        """
        # Get workflow and check if it's active
        workflow = workflow_version.workflow
        if not workflow.is_active:
            raise ValidationError(
                f"Cannot create run for inactive workflow {workflow.id}. "
                f"Please activate the workflow first."
            )

        # Check for existing run with same idempotency_key
        if idempotency_key:
            existing_run = Run.objects.filter(
                idempotency_key=idempotency_key, workflow_version=workflow_version
            ).first()
        ...
        if check_limits:
            ...
            if max_concurrent is None:
                max_concurrent = getattr(
                    settings, "WORKFLOW_MAX_CONCURRENT_RUNS_DEFAULT", 10
                )
    ...
    def start_run(self, run: Run) -> Run:
        ...
        with transaction.atomic():
            ...
            logger.info(f"Started run {run.id}", extra={"run_id": str(run.id)})
    ...
    def handle_step_completion(
        self, run_step: RunStep, outputs: Dict[str, Any]
    ) -> RunStep:
        """
        ...
        with transaction.atomic():
            ...
            run_logger = RunLogger(run_step.run, run_step)
            duration = (
                (run_step.completed_at - run_step.started_at).total_seconds()
                if run_step.started_at
                else None
            )
            run_logger.info(
    ...
    def handle_step_failure(self, run_step: RunStep, error_message: str) -> RunStep:
        """
        ...
        with transaction.atomic():
            ...
            log_state_transition(
                run_step, from_status, "failed", {"error_message": error_message}
            )
    ...
    def complete_run(self, run: Run) -> Run:
        ...
        with transaction.atomic():
            ...
            run.save(
                update_fields=["status", "completed_at", "output_data", "updated_at"]
            )
            ...
            logger.info(f"Completed run {run.id}", extra={"run_id": str(run.id)})
    ...
    def fail_run(self, run: Run, error_message: str) -> Run:
        ...
        with transaction.atomic():
            ...
            log_state_transition(
                run, from_status, "failed", {"error_message": error_message}
            )
