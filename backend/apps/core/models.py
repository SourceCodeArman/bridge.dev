...
class CustomConnector(models.Model):
    ...
    slug = models.SlugField(
        max_length=200,
        help_text='Unique slug for this connector within the workspace'
    )
    display_name = models.CharField(
        max_length=200,
        help_text='Human-readable name for this connector'
    )
    description = models.TextField(
        blank=True,
        help_text='Description of what this connector does'
    )
...
class CustomConnectorVersion(models.Model):
    ...
    manifest = models.JSONField(
        help_text='Connector manifest JSON for this version'
    )
    changelog = models.TextField(
        blank=True,
        help_text='Optional changelog or notes for this version'
    )
    status = models.CharField(
        max_length=20,
        choices=STATUS_CHOICES,
...
class AlertConfiguration(models.Model):
    ...
    alert_on_failure = models.BooleanField(
        default=True,
        help_text='Send alert when workflow run fails'
    )
    alert_on_timeout = models.BooleanField(
        default=True,
        help_text='Send alert when workflow run times out'
    )
    ...
    email_recipients = models.JSONField(
        default=list,
        help_text='List of email addresses to receive alerts'
    )
    slack_webhook_url = models.CharField(
        max_length=500,
        blank=True,
        help_text='Slack webhook URL for alerts'
    )
    webhook_url = models.CharField(
        max_length=500,
        blank=True,
        help_text='Webhook URL for alerts'
    )
    throttle_minutes = models.PositiveIntegerField(
        default=15,
        help_text='Throttle window in minutes (prevent duplicate alerts)'
    )
...
class AlertHistory(models.Model):
    ...
    error_message = models.TextField(
        blank=True,
        help_text='Error message if alert sending failed'
    )
...
class ErrorSuggestion(models.Model):
    ...
    actionable = models.BooleanField(
        default=True,
        help_text='Whether this suggestion can be automatically applied'
    )
...
class WorkflowComment(models.Model):
    ...
    def __str__(self):
        target = f"node {self.node_id}" if self.node_id else f"edge {self.edge_id}" if self.edge_id else "workflow"
