...
function parseConnectors(markdown) {
  const lines = markdown.split('\n');
  const startIndex = lines.findIndex(l => l.includes('## System Connector Functionality Checklist'));
  ...
  // Regex for connector header: #### Name (`slug`)
  const connectorRegex = /^#{4} (.+) \(`(.+)`\)/;
  // Regex for category: ### Category
  const categoryRegex = /^#{3} (.+)/;
  // Regex for checkbox: - [ ] Action or - [x] Action
  const checkboxRegex = /^(\s*)- \[( |x|~)\] (.+)/;

  for (let i = startIndex; i < lines.length; i++) {
    const line = lines[i];

    // Stop if we hit a new major section (## ) but allow the checklist section itself
    if (line.startsWith('## ') && !line.includes('System Connector Functionality Checklist')) {
      break;
    }
    ...
    const connMatch = connectorRegex.exec(line);
    if (connMatch) {
      const name = connMatch[1].trim();
      const slug = connMatch[2].trim();
      currentConnector = {
        name,
        slug,
      ...
      connectors.set(slug, currentConnector);
      continue;
    }

    if (currentConnector) {
      // Capture Auth Type
      if (line.includes('**Auth Type**:')) {
        currentConnector.authType = line.split('**Auth Type**:')[1].trim();
      }

      // Capture Actions/Checkboxes
      const checkMatch = checkboxRegex.exec(line);
      if (checkMatch) {
        const [, indentStr, statusChar, text] = checkMatch;
        const isDone = statusChar === 'x';
        const isProgress = statusChar === '~';
        const indentLevel = Math.floor(indentStr.length / 2); // Roughly 2 spaces per level

        currentConnector.actions.push({
          text: text.trim(),
          done: isDone,
...
function printConnectorDetails(connector) {
  console.log(bold(`${connector.name} (${colors.cyan}${connector.slug}${colors.reset})`));
  console.log(`Category: ${connector.category}`);
  console.log(`Auth Type: ${connector.authType}`);
  console.log(bold('\nActions:'));

  if (connector.actions.length === 0) {
    console.log(colors.gray + '  No actions listed.' + colors.reset);
    return;
  }

  for (const action of connector.actions) {
    let icon = action.done ? '✅' : action.inProgress ? '🔄' : '☐';
    let color = action.done ? colors.green : action.inProgress ? colors.yellow : colors.reset;

    // Indent based on hierarchy
    // Base indent is 2 spaces. Nested items get more.
    const indentPoints = action.indent * 2 + 2;
    const space = ' '.repeat(indentPoints);

    console.log(`${space}${icon} ${color}${action.text}${colors.reset}`);
  }
  console.log('');
}

function updateConnectorActionStatus(markdown, connectorSlug, actionIndex, newStatus) {
  ...
  const lines = markdown.split('\n');
  const startIndex = lines.findIndex(l => l.includes('## System Connector Functionality Checklist'));
  if (startIndex === -1) throw new Error('Connector checklist not found');
  ...
  for (let i = startIndex; i < lines.length; i++) {
    ...
    const connMatch = connectorRegex.exec(line);
    if (connMatch) {
      currentSlug = connMatch[2].trim();
      // Reset action count when we switch connectors
      if (currentSlug !== connectorSlug) {
        actionCount = 0;
      }
      continue;
    }
    ...
    if (currentSlug === connectorSlug) {
      ...
      if (checkMatch) {
        ...
        if (actionCount === actionIndex) {
          actionText = checkMatch[3].trim();
          const newLine = line.replace(/^(\s*)- \[( |x|~)\]/, `$1- [${marker}]`);
          if (newLine !== line) {
            lines[i] = newLine;
            updated = true;
          }
          // We found the target action
          break;
        }
      }
    }
  ...
  if (currentSlug !== connectorSlug && actionCount === 0) {
    // If we never found the connector or acted on it (this logic is a bit loose because loop breaks on find,
    // but let's rely on the pre-check in main for existence, and here just error if not found)
    // Actually, better to flag if we didn't find the action.
  }

  if (actionIndex > actionCount) {
    throw new Error(`Action ${actionIndex} not found for connector ${connectorSlug}. Found ${actionCount} actions.`);
  }

  if (!updated && actionText) {
    // If text found but not updated, it means it was already in that state
    // We can just proceed or throw, but let's just return.
  }

  return { markdown: lines.join('\n'), actionText };
}

function updateAllConnectorActionsStatus(markdown, connectorSlug, newStatus) {
  // Map status to checkbox marker
  const statusMap = {
    'done': 'x',
    'in-progress': '~',
...
function parseArgs() {
  ...
  for (const arg of args) {
    ...
    } else if (arg.startsWith('--subtask=')) {
      parsed.subtask = parseInt(arg.split('=')[1], 10);
    } else if (arg.startsWith('--action=')) {
      parsed.action = parseInt(arg.split('=')[1], 10);
    } else if (arg === '--next-connector') {
      parsed.nextConnector = true;
    } else if (arg.startsWith('--connector=')) {
      parsed.connectorStr = arg.split('=')[1];
...
function updateTaskStatus(markdown, taskId, newStatus, alsoUpdateSubtasks = true) {
  // Map status to checkbox marker
  const statusMap = {
    'done': 'x',
    'in-progress': '~',
  ...
  // If marking as done and alsoUpdateSubtasks is true, mark all subtasks as done
  if (newStatus === 'done' && alsoUpdateSubtasks && taskLineIndex >= 0) {
    ...
    while (j < lines.length) {
      ...
      if (subtaskMatch) {
        ...
        lines[j] = nextLine.replace(/^(\s*)- \[( |x|~)\]/, `$1- [x]`);
        j++;
      } else if (nextLine.trim() === '' || nextLine.match(/^##/)) {
...
function updateSubtaskStatus(markdown, taskId, subtaskIndex, newStatus) {
  // Map status to checkbox marker
  const statusMap = {
    'done': 'x',
    'in-progress': '~',
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    const taskMatch = taskLineRegex.exec(line.trim());

    if (taskMatch) {
      const [, , idStr] = taskMatch;
      const id = Number(idStr);

      if (id === taskId) {
        foundTask = true;
        // Now look for subtasks
        let j = i + 1;
        while (j < lines.length) {
          const nextLine = lines[j];
          const subtaskMatch = subtaskRegex.exec(nextLine);
          ...
          if (subtaskMatch) {
            ...
            if (subtaskCount === subtaskIndex) {
              subtaskTitle = subtaskMatch[3].trim();
              const newLine = nextLine.replace(/^(\s*)- \[( |x|~)\]/, `$1- [${marker}]`);
              if (newLine !== nextLine) {
                lines[j] = newLine;
                updated = true;
              }
            ...
            j++;
          } else if (nextLine.trim() === '' || nextLine.match(/^##/) || taskLineRegex.exec(nextLine.trim())) {
            break;
          } else {
            j++;
          }
  ...
  if (subtaskIndex > subtaskCount) {
    throw new Error(`Subtask ${subtaskIndex} not found. Task ${taskId} only has ${subtaskCount} subtask(s)`);
  }

  if (!updated) {
    throw new Error(`Subtask ${subtaskIndex} is already marked as ${newStatus}`);
  }
...
function printHelp() {
  ...
  console.log('  node tasks-cli.js --next-connector                    # Show next incomplete connector');
  console.log('  node tasks-cli.js --connector=SLUG --action=M --set-status=S  # Update connector action status');
  console.log('  node tasks-cli.js --id=N --set-status=STATUS          # Update task status');
  console.log('  node tasks-cli.js --id=N --subtask=M --set-status=S   # Update subtask status');
  ...
  console.log('  --subtask=M         Subtask index (1-based) to update within the task');
  console.log('  --set-status=STATUS Status to set: done, in-progress, or pending');
  console.log('  --next-connector    Show the first connector with incomplete actions');
  console.log('  --connector=SLUG    Show details for connector (e.g., openai-model)');
  console.log('  --action=M          Action index (1-based) to update within the connector');
  console.log('  --show-task=N       Show detailed information about a task');
  console.log('  --task=N            Alias for --show-task=N');
  console.log('  --help, -h          Show this help message\n');
  console.log('Notes:');
  console.log('  - When a task is marked as done, all its subtasks are also marked as done');
  console.log('  - Subtask indices are 1-based (first subtask is --subtask=1)\n');
  console.log('Examples:');
  console.log(`  ${colors.cyan}node tasks-cli.js --id=1 --set-status=done${colors.reset}           # Mark task 1 and its subtasks as done`);
  console.log(`  ${colors.cyan}node tasks-cli.js --id=5 --set-status=in-progress${colors.reset}   # Mark task 5 as in progress`);
  console.log(`  ${colors.cyan}node tasks-cli.js --id=3 --subtask=2 --set-status=done${colors.reset}  # Mark subtask 2 of task 3 as done`);
  console.log(`  ${colors.cyan}node tasks-cli.js --show-task=1${colors.reset}`);
  console.log(`  ${colors.cyan}node tasks-cli.js --next-connector${colors.reset}`);
  console.log(`  ${colors.cyan}node tasks-cli.js --connector=openai-model${colors.reset}`);
  console.log(`  ${colors.cyan}node tasks-cli.js --connector=gemini --action=2 --set-status=done${colors.reset}`);
...
function main() {
  ...
  // Handle connector flags
  if (args.nextConnector || args.connectorStr) {
    let markdown = fs.readFileSync(tasksPath, 'utf8');
    const connectors = parseConnectors(markdown);

    if (args.connectorStr && args.status) {
      try {
        const connector = connectors.get(args.connectorStr);
        if (!connector) {
          console.error(colors.red + `Connector '${args.connectorStr}' not found.` + colors.reset);
          process.exit(1);
        }
        ...
        if (args.action) {
          ...
          const result = updateConnectorActionStatus(markdown, args.connectorStr, args.action, args.status);
          resultMarkdown = result.markdown;

          // Write back
          fs.writeFileSync(tasksPath, resultMarkdown, 'utf8');
          ...
          console.log(bold('Connector Action Updated Successfully!\n'));
          console.log(`  Connector: ${bold(connector.name)} (${colors.cyan}${args.connectorStr}${colors.reset})`);
          console.log(`  Action ${colors.magenta}${args.action}${colors.reset}: ${result.actionText}`);
          console.log(`  Status: ${statusDisplay}\n`);

        } else {
          // Bulk update all actions
          const result = updateAllConnectorActionsStatus(markdown, args.connectorStr, args.status);
          resultMarkdown = result.markdown;

          // Write back
          fs.writeFileSync(tasksPath, resultMarkdown, 'utf8');

          const statusDisplay = args.status === 'done' ? colors.green + '✅ marked as done' + colors.reset :
            args.status === 'in-progress' || args.status === 'inprogress' ?
              colors.yellow + '🔄 marked as in progress' + colors.reset :
              colors.gray + '⏳ marked as pending' + colors.reset;

          console.log(bold('Connector Bulk Update Successful!\n'));
          console.log(`  Connector: ${bold(connector.name)} (${colors.cyan}${args.connectorStr}${colors.reset})`);
          console.log(`  Updated ${bold(result.updateCount)} actions to: ${statusDisplay}\n`);
        }

        // Re-print connector details to show new state
        const updatedConnectors = parseConnectors(resultMarkdown);
        printConnectorDetails(updatedConnectors.get(args.connectorStr));
    ...
    if (args.connectorStr) {
      const connector = connectors.get(args.connectorStr);
      if (!connector) {
        console.error(colors.red + `Connector '${args.connectorStr}' not found.` + colors.reset);
        console.error(colors.gray + 'Available connectors: ' + Array.from(connectors.keys()).join(', ') + colors.reset);
        process.exit(1);
      }
      ...
      let found = null;
      for (const [slug, conn] of connectors) {
        const hasPending = conn.actions.some(a => !a.done);
        if (hasPending) {
          found = conn;
          break;
        }
      ...
      if (found) {
        console.log(bold('👉 Next Connector to Work On:\n'));
        printConnectorDetails(found);
      } else {
        console.log(colors.green + '🎉 All connectors are complete!' + colors.reset);
      }
    }
    return;
  }
  ...
  if (args.id !== undefined && args.subtask !== undefined && args.status) {
    try {
      let markdown = fs.readFileSync(tasksPath, 'utf8');

      // Update subtask status
      const result = updateSubtaskStatus(markdown, args.id, args.subtask, args.status);
